  ...
  // 1.8  ∫ (2x³ − √x + 4)/√x dx
  {
    ...
    steps: [
      ...
      },
      {
        ...
        tokens: [
          { latex: `${co("2")}x^{${po("5/2")}}`, hint: { term: "x³/√x", text: "x³ : x^{1/2} = x^{3−1/2} = x^{5/2}. При діленні степенів показники віднімаються." } },
          { latex: "-" },
          { latex: `${co("1")}`, hint: { term: "стала 1", text: "Середній доданок став просто одиницею." } },
          { latex: "+" },
          { latex: `${co("4")}x^{${po("-1/2")}}`, hint: { term: "4/√x", text: "1/√x = x^{−1/2}." } },
        ],
        explanation:
          "√x = x^{1/2}. Ділення степенів — віднімання показників: x³/x^{1/2} = x^{5/2}, 1/x^{1/2} = x^{−1/2}.",
        methodIds: ["rewrite"],
        help: {
          plain: "Корінь — це степінь ½. Дріб 1/√x — це x у мінус половині. Тоді працює звичайне степеневе правило.",
          simpler: "Запам'ятай дві речі: √x = x^{1/2}, а «поділити на» = «мінус у показнику». Усе інше — арифметика з показниками.",
          numeric: [
      ...
      },
      {
        ...
        tokens: [
          ...
          { latex: "+" },
          { latex: `${co("4")}\\cdot\\frac{x^{${po("1/2")}}}{${po("1/2")}}`, hint: { term: "x^{−1/2} → x^{1/2}", text: "Показник +1: −1/2+1 = 1/2. Ділимо на 1/2." } },
        ],
        ...
        help: {
          ...
          check: { question: "Чому дорівнює ∫x^{−1/2} dx?", answer: "2x^{1/2} + C" },
        },
      },
      {
        ...
        tokens: [
          ...
          { latex: `x` },
          { latex: "+" },
          { latex: `${co("8")}\\sqrt{x}`, hint: { term: "4 : 1/2 = 8", text: "4 поділити на 1/2 = 8, а x^{1/2} = √x." } },
          { latex: "+" },
          { latex: "C", hint: hintC },
        ],
