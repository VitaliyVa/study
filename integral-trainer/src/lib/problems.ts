    ...
    steps: [
      ...
      },
      {
        ...
        help: {
          ...
          simpler: "Показник = скільки іксів перемножуємо: x³ = x·x·x. Корінь — це «пів ікса»: два корені дають цілий ікс (√x·√x = x), тому √x = x^½. Ділення — це викреслювання: у x⁵ ÷ x² два ікси знизу викреслюють два зверху → лишається x³ (тому 5−2 = 3). А все, що стоїть унизу дробу, піднімається нагору з мінусом: 1/√x = x^(−½).",
          numeric: [
            { latex: "\\sqrt{x}\\cdot\\sqrt{x} = x", note: "два корені = цілий x, тому корінь = пів степеня (½+½=1)" },
            { latex: "\\frac{x\\cdot x\\cdot x\\cdot x\\cdot x}{x\\cdot x} = x^{3}", note: "викреслили 2 ікси знизу → 5−2 = 3" },
            { latex: "\\frac{1}{x^{2}} = x^{-2}", note: "усе, що знизу, піднімається з мінусом" },
          ],
          examples: [
            { latex: "\\sqrt{x} = x^{1/2}", note: "корінь = пів степеня" },
            { latex: "\\frac{1}{\\sqrt{x}} = x^{-1/2}", note: "знизу → мінус показник" },
          ],
          check: { question: "Перепиши 1/√x як степінь.", answer: "x^{-1/2}" },
        },
      },
      {
